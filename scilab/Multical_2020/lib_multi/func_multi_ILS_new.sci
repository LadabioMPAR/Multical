function flag = func_multi_ILS_new(Selecao,optkini,lini,kmax,nc,cname,unid,x0,absor0,frac_test,dadosteste,OptimModel,pretreat,analysis,outlier,gravarXval)
    ...
    case 'Val' then
        ...
        [lixo,indval]=gsort(rand(nd,1));
        indval = indval(1:floor(nd*frac_val));
        Xval = x(indval,:);
        xaj = x;
        xaj(indval,:)=[];
        absorval = absor(indval,:);
        absoraj=absor;
        absoraj(indval,:)=[];
        nval = size(Xval,1);
    ...
    [fd, err] = mopen('Erro_cal.txt' , 'w')
    mfprintf(fd,formato0,cname);
    mfprintf(fd,formato,[[1:kmax]' RMSEcalconc]);
    [minimos,ind] = min(RMSECVconc,'r');
    mclose(fd)
    for h = 1:size(ind,2)
        minimo(1,h) = RMSECVconc(ind(h), h)
    end
    minimos = minimo;
    minimos(2,:) = ind;
    fprintfMat('minimos.txt',minimos )
    ...
    if kpart - cont > 0
        warning('Número real de partições de k-fold é')
        warning( string(cont))
    end
    flag = 0; // saída
endfunction
